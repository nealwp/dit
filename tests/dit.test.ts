...
describe('dit', () => {
    ...
    it('should delete an entry', async () => {
        const inserted = await db.all(
            'insert into task (description, date_added) values ("delete me", "2023-02-03") returning rowid;',
        );
    ...
    it('should get todays entries', async () => {
        await db.all(
            'insert into task (description, date_added) values ("yep", date("now")), ("yes", date("now")), ("nope", "1970-01-01");',
        );
    ...
    it('should get current months entries', async () => {
        await db.all(
            'insert into task (description, date_added) values ("foo", date("now")), ("bar", date("now")), ("baz", date("now", "-1 day")), ("ancient", date("now", "-999 days"));',
        );
    ...
    it('should edit an existing entry', async () => {
        const inserted = await db.all(
            'insert into task (description, date_added) values ("edit me", "2023-02-03") returning rowid;',
        );
